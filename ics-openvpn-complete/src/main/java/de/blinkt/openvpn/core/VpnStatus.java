...
public class VpnStatus {
    ...
    private static int mLastStateresid = R.string.openvpn_state_noprocess;
    ...
    public static void logException(LogLevel ll, String context, Exception e) {
        ...
        LogItem li;
        if (context != null) {
            li = new LogItem(ll, R.string.openvpn_unhandled_exception_context, e.getMessage(), sw.toString(), context);
        } else {
            li = new LogItem(ll, R.string.openvpn_unhandled_exception, e.getMessage(), sw.toString());
        }
        newLogItem(li);
    }
        ...
        public String getString(Context c) {
            try {
                if (mMessage != null) {
                    ...
                    if (c != null) {
                        if (mRessourceId == R.string.openvpn_mobile_info)
        ...
        private String getMobileInfoString(Context c) {
            ...
            try {
                ...
                if (Arrays.equals(digest, officalkey))
                    apksign = c.getString(R.string.openvpn_official_build);
                else if (Arrays.equals(digest, officaldebugkey))
                    apksign = c.getString(R.string.openvpn_debug_build);
                    ...
                    apksign = c.getString(R.string.openvpn_built_by, cert.getSubjectX500Principal().getName());
            ...
            return c.getString(R.string.openvpn_mobile_info_extended, argsext);
    ...
    private static void logInformation() {
        logInfo(R.string.openvpn_mobile_info, Build.MODEL, Build.BOARD, Build.BRAND, Build.VERSION.SDK_INT);
    }
    ...
    private static int getLocalizedState(String state) {
        if (state.equals("CONNECTING"))
            return R.string.openvpn_state_connecting;
        else if (state.equals("WAIT"))
            return R.string.openvpn_state_wait;
        else if (state.equals("AUTH"))
            return R.string.openvpn_state_auth;
        else if (state.equals("GET_CONFIG"))
            return R.string.openvpn_state_get_config;
        else if (state.equals("ASSIGN_IP"))
            return R.string.openvpn_state_assign_ip;
        else if (state.equals("ADD_ROUTES"))
            return R.string.openvpn_state_add_routes;
        else if (state.equals("CONNECTED"))
            return R.string.openvpn_state_connected;
        else if (state.equals("DISCONNECTED"))
            return R.string.openvpn_state_disconnected;
        else if (state.equals("RECONNECTING"))
            return R.string.openvpn_state_reconnecting;
        else if (state.equals("EXITING"))
            return R.string.openvpn_state_exiting;
        else if (state.equals("RESOLVE"))
            return R.string.openvpn_state_resolve;
        else if (state.equals("TCP_CONNECT"))
            return R.string.openvpn_state_tcp_connect;
        else
            return R.string.openvpn_unknown_state;
    ...
    public static void updateStatePause(OpenVPNManagement.pauseReason pauseReason) {
        switch (pauseReason) {
            case noNetwork:
                VpnStatus.updateStateString("NONETWORK", "", R.string.openvpn_state_nonetwork, ConnectionStatus.LEVEL_NONETWORK);
                break;
            case screenOff:
                VpnStatus.updateStateString("SCREENOFF", "", R.string.openvpn_state_screenoff, ConnectionStatus.LEVEL_VPNPAUSED);
                break;
            case userPause:
                VpnStatus.updateStateString("USERPAUSE", "", R.string.openvpn_state_userpause, ConnectionStatus.LEVEL_VPNPAUSED);
                break;
        }
