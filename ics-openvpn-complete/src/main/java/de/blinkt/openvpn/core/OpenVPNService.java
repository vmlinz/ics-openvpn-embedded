...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener {
    ...
    private void showNotification(String msg, String tickerText, boolean lowpriority, long when, ConnectionStatus status) {
        ...
        if (mProfile != null)
            nbuilder.setContentTitle(getString(R.string.openvpn_notifcation_title, mProfile.mName));
        else
            nbuilder.setContentTitle(getString(R.string.openvpn_notifcation_title_notconnect));
    ...
    private int getIconByConnectionStatus(ConnectionStatus level) {
        switch (level) {
            case LEVEL_CONNECTED:
                return R.drawable.openvpn_ic_stat_vpn;
            case LEVEL_AUTH_FAILED:
            case LEVEL_NONETWORK:
            case LEVEL_NOTCONNECTED:
                return R.drawable.openvpn_ic_stat_vpn_offline;
            case LEVEL_CONNECTING_NO_SERVER_REPLY_YET:
            case LEVEL_WAITING_FOR_USER_INPUT:
                return R.drawable.openvpn_ic_stat_vpn_outline;
            case LEVEL_CONNECTING_SERVER_REPLIED:
                return R.drawable.openvpn_ic_stat_vpn_empty_halo;
    ...
    private void jbNotificationExtras(boolean lowpriority,
        ...
        try {
            ...
            nbuilder.addAction(android.R.drawable.ic_menu_close_clear_cancel,
                    getString(R.string.openvpn_cancel_connection), disconnectPendingIntent);
            ...
            if (mDeviceStateReceiver == null || !mDeviceStateReceiver.isUserPaused()) {
                ...
                nbuilder.addAction(android.R.drawable.ic_media_pause,
                        getString(R.string.openvpn_pauseVPN), pauseVPNPending);

            } else {
                pauseVPN.setAction(RESUME_VPN);
                PendingIntent resumeVPNPending = PendingIntent.getService(this, 0, pauseVPN, 0);
                nbuilder.addAction(android.R.drawable.ic_media_play,
                        getString(R.string.openvpn_resumevpn), resumeVPNPending);
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent == null) {
            ...
            VpnStatus.logInfo(R.string.openvpn_service_restarted);
        ...
        String startTitle = getString(R.string.openvpn_start_vpn_title, mProfile.mName);
        String startTicker = getString(R.string.openvpn_start_vpn_ticker, mProfile.mName);
    ...
    public ParcelFileDescriptor openTun() {
        ...
        VpnStatus.logInfo(R.string.openvpn_last_openvpn_tun_config);
        ...
        if (mLocalIP == null && mLocalIPv6 == null) {
            VpnStatus.logError(getString(R.string.openvpn_opentun_no_ipaddr));
            return null;
        }
        ...
        if (mLocalIP != null) {
            ...
            try {
                ...
                VpnStatus.logError(R.string.openvpn_dns_add_error, mLocalIP, iae.getLocalizedMessage());
                return null;
            }
        }
        ...
        if (mLocalIPv6 != null) {
            ...
            try {
                ...
                VpnStatus.logError(R.string.openvpn_ip_add_error, mLocalIPv6, iae.getLocalizedMessage());
                return null;
            }
        ...
        for (String dns : mDnslist) {
            try {
                ...
                VpnStatus.logError(R.string.openvpn_dns_add_error, dns, iae.getLocalizedMessage());
            }
        }
        ...
        for (NetworkSpace.ipAddress route : positiveIPv4Routes) {
            try {
                ...
                if (multicastRange.containsNet(route))
                    VpnStatus.logDebug(R.string.openvpn_ignore_multicast_route, route.toString());
                else
                    builder.addRoute(route.getIPv4Address(), route.networkMask);
            } catch (IllegalArgumentException ia) {
                VpnStatus.logError(getString(R.string.openvpn_route_rejected) + route + " " + ia.getLocalizedMessage());
            }
        }
        ...
        for (NetworkSpace.ipAddress route6 : positiveIPv6Routes) {
            try {
                ...
                VpnStatus.logError(getString(R.string.openvpn_route_rejected) + route6 + " " + ia.getLocalizedMessage());
            }
        }
        ...
        VpnStatus.logInfo(R.string.openvpn_local_ip_info, mLocalIP.mIp, mLocalIP.len, mLocalIPv6, mMtu);
        VpnStatus.logInfo(R.string.openvpn_dns_server_info, TextUtils.join(", ", mDnslist), mDomain);
        VpnStatus.logInfo(R.string.openvpn_routes_info_incl, TextUtils.join(", ", mRoutes.getNetworks(true)), TextUtils.join(", ", mRoutesv6.getNetworks(true)));
        VpnStatus.logInfo(R.string.openvpn_routes_info_excl, TextUtils.join(", ", mRoutes.getNetworks(false)), TextUtils.join(", ", mRoutesv6.getNetworks(false)));
        VpnStatus.logDebug(R.string.openvpn_routes_debug, TextUtils.join(", ", positiveIPv4Routes), TextUtils.join(", ", positiveIPv6Routes));
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        if (mLocalIP != null && mLocalIPv6 != null)
            session = getString(R.string.openvpn_session_ipv6string, session, mLocalIP, mLocalIPv6);
        else if (mLocalIP != null)
            session = getString(R.string.openvpn_session_ipv4string, session, mLocalIP);

        builder.setSession(session);

        // No DNS Server, log a warning
        if (mDnslist.size() == 0)
            VpnStatus.logInfo(R.string.openvpn_warn_no_dns);
        ...
        try {
            ...
            VpnStatus.logError(R.string.openvpn_tun_open_error);
            VpnStatus.logError(getString(R.string.openvpn_error) + e.getLocalizedMessage());
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                VpnStatus.logError(R.string.openvpn_tun_error_helpful);
            }
            return null;
        }
    ...
    private void setAllowedVpnPackages(Builder builder) {
        for (String pkg : mProfile.mAllowedAppsVpn) {
            try {
                ...
                VpnStatus.logInfo(R.string.openvpn_app_no_longer_exists, pkg);
            }
        }

        if (mProfile.mAllowedAppsVpnAreDisallowed) {
            VpnStatus.logDebug(R.string.openvpn_disallowed_vpn_apps_info, TextUtils.join(", ", mProfile.mAllowedAppsVpn));
        } else {
            VpnStatus.logDebug(R.string.openvpn_allowed_vpn_apps_info, TextUtils.join(", ", mProfile.mAllowedAppsVpn));
        }
    }
    ...
    public void addRoute(String dest, String mask, String gateway, String device) {
        ...
        if (route.len == 32 && !mask.equals("255.255.255.255")) {
            VpnStatus.logWarning(R.string.openvpn_route_not_cidr, dest, mask);
        }

        if (route.normalise())
            VpnStatus.logWarning(R.string.openvpn_route_not_netip, dest, route.len, route.mIp);
    ...
    public void setLocalIP(String local, String netmask, int mtu, String mode) {
        ...
        if (mLocalIP.len == 32 && !netmask.equals("255.255.255.255")) {
            ...
            if ((netMaskAsInt & mask) == (mLocalIP.getInt() & mask)) {
                ...
                mLocalIP.len = 32;
                if (!"p2p".equals(mode))
                    VpnStatus.logWarning(R.string.openvpn_ip_not_cidr, local, netmask, mode);
            }
        }
        if (("p2p".equals(mode) && mLocalIP.len < 32) || ("net30".equals(mode) && mLocalIP.len < 30)) {
            VpnStatus.logWarning(R.string.openvpn_ip_looks_like_subnet, local, netmask, mode);
        }
    ...
    public void updateByteCount(long in, long out, long diffIn, long diffOut) {
        if (mDisplayBytecount) {
            String netstat = String.format(getString(R.string.openvpn_statusline_bytecount),
