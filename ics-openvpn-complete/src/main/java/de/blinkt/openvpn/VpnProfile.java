...
public class VpnProfile implements Serializable, Cloneable {
            ...
            case VpnProfile.TYPE_KEYSTORE:
                if (!configForOvpn3) {
                    ...
                    if (ks != null) {
                        ...
                        cfg += context.getString(R.string.openvpn_keychain_access) + "\n";
                        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.JELLY_BEAN)
                            if (!mAlias.matches("^[a-zA-Z0-9]$"))
                                cfg += context.getString(R.string.openvpn_jelly_keystore_alphanumeric_bug) + "\n";
                    }
                }
                break;
    ...
    synchronized String[] getKeyStoreCertificates(Context context,int tries) {
        ...
        try {
            ...
            if (caChain.length <= 1 && TextUtils.isEmpty(mCaFilename)) {
                VpnStatus.logMessage(VpnStatus.LogLevel.ERROR, "", context.getString(R.string.openvpn_keychain_nocacert));
            } else {
        ...
        exp.printStackTrace();
        VpnStatus.logError(R.string.openvpn_keyChainAccessError, exp.getLocalizedMessage());

        VpnStatus.logError(R.string.openvpn_keychain_access);
        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.JELLY_BEAN) {
            if (!mAlias.matches("^[a-zA-Z0-9]$")) {
                VpnStatus.logError(R.string.openvpn_jelly_keystore_alphanumeric_bug);
            }
        }
        return null;
    }
    ...
    public int checkProfile(Context context) {
        ...
        if(noRemoteEnabled)
            return R.string.openvpn_remote_no_server_selected;

        // Everything okay
        return R.string.openvpn_no_error_found;
    ...
    public int needUserPWInput(boolean ignoreTransient) {
        if ((mAuthenticationType == TYPE_PKCS12 || mAuthenticationType == TYPE_USERPASS_PKCS12) &&
            ...
            if (ignoreTransient || mTransientPCKS12PW == null)
                return R.string.openvpn_pkcs12_file_encryption_key;
        }

        if (mAuthenticationType == TYPE_CERTIFICATES || mAuthenticationType == TYPE_USERPASS_CERTIFICATES) {
            if (requireTLSKeyPassword() && TextUtils.isEmpty(mKeyPassword))
                if (ignoreTransient || mTransientPCKS12PW == null) {
                    return R.string.openvpn_private_key_password;
                }
        }
        ...
        if (isUserPWAuth() &&
            ...
            return R.string.openvpn_password;
        }
        return 0;
    }
    ...
    public String getSignedData(String b64data) {
        ...
        VpnStatus.logError(R.string.openvpn_error_rsa_sign, err.getClass().toString(), err.getLocalizedMessage());
