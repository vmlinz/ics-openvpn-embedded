...
public class RemoteCNPreference extends DialogPreference {
	...
	public RemoteCNPreference(Context context, AttributeSet attrs) {
		super(context, attrs);
		setDialogLayoutResource(R.layout.openvpn_tlsremote);
	...
	protected void onBindDialogView(View view) {
		...
		mEditText = (EditText) view.findViewById(R.id.openvpn_tlsremotecn);
		mSpinner = (Spinner) view.findViewById(R.id.openvpn_x509verifytype);
		mRemoteTLSNote = (TextView) view.findViewById(R.id.openvpn_tlsremotenote);
		//mScrollView = (ScrollView) view.findViewById(R.id.openvpn_tlsremotescroll);
		if(mDn!=null)
	...
	private void populateSpinner() {
		...
		authtypes.add(getContext().getString(R.string.openvpn_complete_dn));
		authtypes.add(getContext().getString(R.string.openvpn_rdn));
		authtypes.add(getContext().getString(R.string.openvpn_rdn_prefix));
		if ((mDNType == VpnProfile.X509_VERIFY_TLSREMOTE || mDNType == VpnProfile.X509_VERIFY_TLSREMOTE_COMPAT_NOREMAPPING)
				&& !(mDn==null || "".equals(mDn))) {
			authtypes.add(getContext().getString(R.string.openvpn_tls_remote_deprecated));
