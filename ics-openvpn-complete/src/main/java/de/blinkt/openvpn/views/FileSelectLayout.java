...
public class FileSelectLayout extends LinearLayout implements OnClickListener {
    ...
    public FileSelectLayout(Context context, AttributeSet attrset) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrset, R.styleable.openvpn_FileSelectLayout);

        setupViews(ta.getString(R.styleable.openvpn_FileSelectLayout_openvpn_fileTitle), ta.getBoolean(R.styleable.openvpn_FileSelectLayout_openvpn_certificate, true),
                    ta.getBoolean(R.styleable.openvpn_FileSelectLayout_openvpn_showClear,false));
    ...
    private void setupViews(String title, boolean isCertificate, boolean showClear) {
        inflate(getContext(), R.layout.openvpn_file_select, this);

        mTitle = title;
        mIsCertificate = isCertificate;

        TextView tview = (TextView) findViewById(R.id.openvpn_file_title);
        tview.setText(mTitle);

        mDataView = (TextView) findViewById(R.id.openvpn_file_selected_item);
        mDataDetails = (TextView) findViewById(R.id.openvpn_file_selected_description);
        mSelectButton = (Button) findViewById(R.id.openvpn_file_select_button);
        mSelectButton.setOnClickListener(this);

        mShowClearButton = (Button) findViewById(R.id.openvpn_file_clear_button);
    ...
    public void setData(String data, Context c) {
        mData = data;
        if (data == null) {
            mDataView.setText(c.getString(R.string.openvpn_no_data));
            mDataDetails.setText("");
            mShowClearButton.setVisibility(GONE);
        } else {
            if (mData.startsWith(VpnProfile.DISPLAYNAME_TAG)) {
                mDataView.setText(c.getString(R.string.openvpn_imported_from_file, VpnProfile.getDisplayName(mData)));
            } else if (mData.startsWith(VpnProfile.INLINE_TAG))
                mDataView.setText(R.string.openvpn_inline_file_data);
            else
