...
public class VPNPreferences extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mProfile = ProfileManager.get(this,mProfileUUID);
		if(mProfile!=null) {
			setTitle(getString(R.string.openvpn_edit_profile_title, mProfile.getName()));
		}
        ...
        if (mProfile.mUserEditable) {
            mPagerAdapter.addTab(R.string.openvpn_basic, Settings_Basic.class);
            mPagerAdapter.addTab(R.string.openvpn_server_list, Settings_Connections.class);
            mPagerAdapter.addTab(R.string.openvpn_ipdns, Settings_IP.class);
            mPagerAdapter.addTab(R.string.openvpn_routing, Settings_Routing.class);
            mPagerAdapter.addTab(R.string.openvpn_settings_auth, Settings_Authentication.class);

            mPagerAdapter.addTab(R.string.openvpn_advanced, Settings_Obscure.class);
        } else {
            mPagerAdapter.addTab(R.string.openvpn_basic, Settings_UserEditable.class);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
            mPagerAdapter.addTab(R.string.openvpn_vpn_allowed_apps, Settings_Allowed_Apps.class);

        mPagerAdapter.addTab(R.string.openvpn_generated_config, ShowConfigFragment.class);
...
/*
	...
	public void onBuildHeaders(List<Header> target) {
		loadHeadersFromResource(R.xml.openvpn_vpn_headers, target);
        Header headerToRemove=null;
        for (Header header : target) {
			if(header.fragmentArguments==null)
				header.fragmentArguments = new Bundle();
			header.fragmentArguments.putString(getPackageName() + ".profileUUID",mProfileUUID);
            if (header.id == R.id.openvpn_allowed_apps_header && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP)
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.openvpn_remove_vpn)
			askProfileRemoval();
        if (item.getItemId() == R.id.openvpn_duplicate_vpn) {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.openvpn_vpnpreferences_menu, menu);
	...
	private void askProfileRemoval() {
		...
		dialog.setMessage(getString(R.string.openvpn_remove_vpn_query, mProfile.mName));
