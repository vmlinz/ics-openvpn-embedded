...
public class Settings_Authentication extends OpenVpnPreferencesFragment implements OnPreferenceChangeListener, OnPreferenceClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(R.xml.openvpn_vpn_authentification);
		...
		if(preference==mRemoteCN) {
			...
			if ("".equals(dn)) {
                if (mProfile.mConnections.length > 0) {
                    ...
                    preference.setSummary(R.string.openvpn_no_remote_defined);
                }
            } else {
    ...
    void startFileDialog() {
        ...
        if (startFC == null) {
            ...
            startFC.putExtra(FileSelect.WINDOW_TITLE, R.string.openvpn_tls_auth_file);
	...
	private void setTlsAuthSummary(String result) {
		if(result==null)
            result = getString(R.string.openvpn_no_certificate);
		if(result.startsWith(VpnProfile.INLINE_TAG))
			mTLSAuthFile.setSummary(R.string.openvpn_inline_file_data);
        else if (result.startsWith(VpnProfile.DISPLAYNAME_TAG))
            mExpectTLSCert.setSummary(getString(R.string.openvpn_imported_from_file, VpnProfile.getDisplayName(result)));
		else
